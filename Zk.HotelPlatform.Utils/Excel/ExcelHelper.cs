...
namespace HotelsSpider.Util
{
    public class ExcelHelper
    {
        ...
        public static MemoryStream ExportMemoryStream(DataTable dtSource, ExcelConfig excelConfig)
        {
            ...
            var cols = excelConfig.ColumnEntity.Select(x => x.Column);
            ...
            #region 右击文件 属性信息
            {
                ...
                si.Author = "系统"; //填加xls文件作者信息
                si.ApplicationName = "众客网络"; //填加xls文件创建程序信息
                si.LastAuthor = "系统"; //填加xls文件最后保存者信息
                si.Comments = "系统"; //填加xls文件作者信息
            ...
            foreach (DataColumn item in dtSource.Columns)
            {
                ...
                catch (Exception ex )
                {
                    ...
                    //MyLog.ErrorLog("execel导出异常1", ex.StackTrace);
                }
            }
            ...
            foreach (DataRow row in dtSource.Rows)
            {
                ...
                foreach (DataColumn column in dtSource.Columns)
                {
                    ...
                    catch (Exception ex )
                    {
                        ...
                        //MyLog.ErrorLog("execel导出异常2", ex.StackTrace);
                    }
                }
                #endregion
                rowIndex++;
        ...
        public static MemoryStream ToExcel<T>(List<T> list, string filePath = null)
        {
            ...
            catch (Exception ex )
            {
                ...
                //MyLog.ErrorLog("excel异常",ex.StackTrace);
            }
            return memoryStream;
        ...
        public static DataTable ReadExcelToTable(string path)//excel存放的路径
        {
            try
            {
                ...
                //连接字符串
                  string connstring = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + path + ";Extended Properties='Excel 12.0;HDR=NO;IMEX=1';"; // Office 07及以上版本 不能出现多余的空格 而且分号注意
